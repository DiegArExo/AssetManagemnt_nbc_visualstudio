...
namespace ITAssetManagement.Controllers
{
    public class desktop_monitorsController : NbcController
    {
        ...
        //-------------------------------------------------DOWNLOAND THE ATTACHMENT(MONITOR WRITTEN OFF DOCUMENT) START----------------------------------------------
       
        [Route("api/desktop_monitors/download_attachment")]
        [HttpGet]
        public IHttpActionResult DownloadAttachment(int monitorId, string token)
        {
            ...
            try
            {
                ...
                var monitor = db.desktop_monitors
                                ...
                                .Select(m => new { m.attachment })
                                .FirstOrDefault();

                if (monitor == null || string.IsNullOrEmpty(monitor.attachment))
                {
                    return NotFound();
                }

                // Clean the base64 string if it contains any prefix like "data:application/pdf;base64,"
        

                // Attempt to convert the base64 encoded attachment to byte array
                byte[] attachmentData;
                try
                {
                    attachmentData = Convert.FromBase64String(CleanBase64String(monitor.attachment));
                }
                catch (FormatException ex)
                {
                    return Content(HttpStatusCode.InternalServerError, new { Message = "Invalid attachment format (Base64).", Details = ex.Message });
                }

       
                string filename = $"monitor_{monitorId}_attachment.pdf";
